...
describe('buildCirclingMessage', () => {
    it('includes aircraft with unknown registration when no r', () => {
        const msg = buildCirclingMessage({ hex: 'ABC123' }, null, url);
    ...
    it('includes registration or icao phrase when r is set', () => {
        const msg = buildCirclingMessage({ hex: 'A1B2C3', r: 'N352HP' }, null, url);
        expect(msg).toMatch(
            /^(#N352HP is circling|Aircraft with unknown registration, hex\/ICAO A1B2C3, is circling)/,
        );
        expect(msg).toContain(url);
    });

    it('includes call sign when flight differs from registration', () => {
        const msg = buildCirclingMessage({ hex: 'ABC', r: 'N123', flight: 'PAT456' }, null, url);
        expect(msg).toContain('call sign #PAT456');
    });

    it('omits call sign when flight equals registration', () => {
        const msg = buildCirclingMessage({ hex: 'ABC', r: 'N123', flight: 'N123' }, null, url);
        expect(msg).not.toContain('call sign');
    });

    it('includes operator when set (circling, reg only): ", operated by X"', () => {
        const msg = buildCirclingMessage({ hex: 'ABC', r: 'N352HP', operator: 'Acme Survey Co' }, null, url);
    ...
    it('includes altitude when alt_baro is set', () => {
        const msg = buildCirclingMessage({ hex: 'ABC', alt_baro: 3500 }, null, url);
        expect(msg).toContain('at 3500 feet');
    });

    it('omits altitude when alt_baro is ground', () => {
        const msg = buildCirclingMessage({ hex: 'ABC', alt_baro: 'ground' }, null, url);
        expect(msg).not.toContain('feet');
    });

    it('includes speed in MPH when gs (knots) is set', () => {
        const msg = buildCirclingMessage({ hex: 'ABC', gs: 100 }, null, url);
    ...
    it('includes squawk when set', () => {
        const msg = buildCirclingMessage({ hex: 'ABC', squawk: '4521' }, null, url);
        expect(msg).toContain('squawking 4521');
    });

    it('includes location from reverse geo (locality)', () => {
        const msg = buildCirclingMessage({ hex: 'ABC' }, { locality: 'Los Angeles' }, url);
    ...
    it('ends with URL', () => {
        const msg = buildCirclingMessage({ hex: 'X', r: 'N1' }, null, 'https://link.example/');
    ...
    it('uses military phrasing when isMilitary and registration', () => {
        const msg = buildCirclingMessage({ hex: 'AE1234', r: '08-1234', isMilitary: true }, null, url);
    ...
    it('uses military unknown registration when isMilitary and no r', () => {
        ...
        const msg = buildCirclingMessage({ hex: 'AE1234', isMilitary: true }, null, url, { random: last });
    ...
    it('uses type with registration when type is set', () => {
        const msg = buildCirclingMessage({ hex: 'A1B2C3', r: 'N123', type: 'Cessna 172' }, null, url);
    ...
    it('uses type with unknown registration when type set and no r', () => {
        const msg = buildCirclingMessage({ hex: 'ABC', type: 'Boeing 737' }, null, url);
        expect(msg).toMatch(
            /(Boeing 737 with unknown registration, hex\/ICAO ABC|Aircraft with unknown registration, hex\/ICAO ABC)/,
        );
    });

    it('includes landmark when provided in options', () => {
        const msg = buildCirclingMessage({ hex: 'ABC' }, { locality: 'LA' }, url, {
    ...
    it('includes fire when provided in options', () => {
        const msg = buildCirclingMessage({ hex: 'ABC' }, null, url, {
...
describe('buildCirclingMessage (deterministic grammar branches)', () => {
    ...
    describe('id_and_type', () => {
        it('exact: registration only when random picks first', () => {
            const msg = buildCirclingMessage({ hex: 'A1B2C3', r: 'N352HP' }, null, url, { random: first });
            expect(msg).toMatch(/^#N352HP is circling\nhttps?:\/\//);
        });

        it('exact: aircraft with unknown registration when no reg and random picks first', () => {
            const msg = buildCirclingMessage({ hex: 'A1B2C3' }, null, url, { random: first });
            expect(msg).toMatch(
                /^Aircraft with unknown registration, hex\/ICAO A1B2C3, is circling\nhttps?:\/\//,
            );
        });

        it('exact: registration + type when type set and first', () => {
            const msg = buildCirclingMessage({ hex: 'X', r: 'N123', type: 'Cessna 172' }, null, url, {
                random: first,
            });
        ...
        it('exact: military + registration (first option)', () => {
            const msg = buildCirclingMessage({ hex: 'AE1', r: '08-1234', isMilitary: true }, null, url, {
                random: first,
            });
        ...
        it('exact: type with unknown registration', () => {
            ...
            expect(msg).toMatch(
                /^Boeing 737 with unknown registration, hex\/ICAO ABC, is circling\nhttps?:\/\//,
            );
        });

        it('exact: military unknown registration', () => {
            const msg = buildCirclingMessage({ hex: 'AE1234', isMilitary: true }, null, url, {
                random: last,
            });
            expect(msg).toMatch(
                /^Military aircraft with unknown registration, hex\/ICAO AE1234, is circling\nhttps?:\/\//,
            );
        });
    });
    ...
    describe('location', () => {
        ...
        it('exact: locality only when only locality set', () => {
            const msg = buildCirclingMessage({ hex: 'X' }, { locality: 'Burbank' }, url, { random: first });
        ...
        it('exact: name/label fallback', () => {
            const msg = buildCirclingMessage({ hex: 'X' }, { name: 'Downtown' }, url, { random: last });
            expect(msg).toContain('circling over Downtown');
        });

        it('unknown location when props empty and no name', () => {
            const msg = buildCirclingMessage({ hex: 'X' }, {}, url, { random: first });
    ...
    describe('optional clauses', () => {
        it('altitude: string number and decimal rounded', () => {
            const msg = buildCirclingMessage({ hex: 'X', alt_baro: '3500' }, null, url, { random: first });
            expect(msg).toContain('at 3500 feet');
        });

        it('altitude: number rounded', () => {
            const msg = buildCirclingMessage({ hex: 'X', alt_baro: 4200.7 }, null, url, { random: first });
            expect(msg).toContain('at 4201 feet');
        });

        it('speed: knots to MPH conversion', () => {
            const msg = buildCirclingMessage({ hex: 'X', gs: 100 }, null, url, { random: first });
            expect(msg).toMatch(/speed 115 MPH/);
        });

        it('landmark distance formatted to one decimal', () => {
            const msg = buildCirclingMessage({ hex: 'X' }, null, url, {
                landmark: { name: 'Stadium', distanceMiles: 2.456 },
                random: first,
            });
        ...
        it('fire distance formatted', () => {
            const msg = buildCirclingMessage({ hex: 'X' }, null, url, {
                fire: { name: 'Fire', distanceMiles: 10 },
                random: first,
            });
    ...
    describe('structure', () => {
        ...
        it('always ends with newline + url', () => {
            const msg = buildCirclingMessage({ hex: 'X' }, null, 'https://link.example', { random: first });
            expect(msg).toMatch(/\nhttps:\/\/link\.example$/);
        });

        it('no trailing/leading junk', () => {
            const msg = buildCirclingMessage({ hex: 'X', r: 'N1' }, { locality: 'LA' }, url, {
                random: first,
            });
            expect(msg).toBe(msg.trim());
    ...
    describe('edge cases', () => {
        it('minimal: hex only, no location', () => {
            const msg = buildCirclingMessage({ hex: 'ABC123' }, null, url, { random: first });
...
describe('buildImagingMessage', () => {
    it('includes operator when set (imaging): ", operated by X" (comma for imaging; no-comma only for circling)', () => {
        const msg = buildImagingMessage({ hex: 'ABC', r: 'N352HP', operator: 'Acme Survey Co' }, null, url);
        expect(msg).toContain(', operated by Acme Survey Co');
        expect(msg).toContain('appears to be on an imaging/survey pattern');
        expect(msg).toMatch(
            /^#N352HP, operated by Acme Survey Co, appears to be on an imaging\/survey pattern\n/,
        );
        expect(msg).toContain(url);
    });
});
...
describe('buildScreenshotAlt', () => {
    it('includes flight and location when present', () => {
        const alt = buildScreenshotAlt({ locality: 'Salt Lake City' }, null, 'N12345');
